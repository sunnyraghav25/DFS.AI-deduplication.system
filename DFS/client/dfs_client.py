...
def upload(ftp, ufile):
    try:
        ...
        f_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), ufile)
        ftp.storbinary("STOR " + ufile, open(f_path, 'rb')) # start uploading the file to DNode FTPserver (opens the file as read-binary in the client)
